...
void executionTimeSort(ProcessBook* pb) { // 按进程执行时间升序排序
	// 冒泡排序: 每次找到从i到pb->m_ProcessArray[i].m_ProcessCount - 1中优先级最高的进程，放到pb->m_ProcessArray[i]
	for (int i = 0; i < pb->m_ProcessCount - 1; i++) { // 按进程创建、执行时间进行升序排序 
	...
	for (int i = 0; i < pb->m_ProcessCount - 1; i++) {
		if (i == 0) { // 由于排序后第一个进程的创建时间最短，所以创建时间就是开始时间 
			pb->m_ProcessArray[i].m_StartTime = pb->m_ProcessArray[i].m_CreationTime;
		}
		else { // 除第一个进程，其余进程的开始时间由上一个进程的完成时间或是此进程的创建时间决定的
		...
		for (int k = i + 1; k < i + count; k++) { // k的取值从i+1到i+count即所有完成时间内已创建的进程 
			if (pb->m_ProcessArray[k + 1].m_ExecutionTime < min) { // 遍历这些进程找到最短进程的下标next 
